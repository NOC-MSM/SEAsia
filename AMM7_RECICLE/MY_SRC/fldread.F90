      ...
      IF ( map%ll_unstruc ) THEN                            ! unstructured open boundary data file
            ...
            SELECT CASE( igrd )                         
               CASE(1)
                  IF( ABS( (zh - ht(zij,zjj)) / ht(zij,zjj)) * tmask(zij,zjj,1) > 0.01_wp ) THEN
                     ...
                     !IF(lwp) WRITE(numout,*) 'DEPTHT', zh, sum(e3t_n(zij,zjj,:), mask=tmask(zij,zjj,:)==1),  ht(zij,zjj), map%ptr(ib), ib, zij, zjj
                  ENDIF
               CASE(2)
                  IF( ABS( (zh - hu(zij,zjj)) * hur(zij,zjj)) * umask(zij,zjj,1) > 0.01_wp ) THEN
                     WRITE(ibstr,"(I10.10)") map%ptr(ib) 
                     ...
                     !  &                hu(zij,zjj), map%ptr(ib), ib, zij, zjj, narea-1  , &
                     !   &                dta_read(map%ptr(ib),1,:)
                  ENDIF
               CASE(3)
                  IF( ABS( (zh - hv(zij,zjj)) * hvr(zij,zjj)) * vmask(zij,zjj,1) > 0.01_wp ) THEN
                     WRITE(ibstr,"(I10.10)") map%ptr(ib) 
         ...
         IF(igrd == 3) THEN                                 ! do we need to adjust the transport term?
            DO ib = 1, ipi
              ...
              END DO
              DO ik = 1, ipk                                ! make transport correction
                 !dta(ib,1,ik) = ( dta(ib,1,ik) + ( ztrans*fvl - ztrans_new ) * hvr(zij,zjj) ) * vmask(zij,zjj,ik)
                 IF(fvl) THEN ! bdy data are total velocity so adjust bt transport term to match input data
                    dta(ib,1,ik) = ( dta(ib,1,ik) + ( ztrans - ztrans_new ) * hvr(zij,zjj) ) * vmask(zij,zjj,ik)
                 ELSE ! we're just dealing with bc velocity so bt transport term should sum to zero
                    dta(ib,1,ik) = dta(ib,1,ik) + ( 0._wp - ztrans_new ) * hvr(zij,zjj) * vmask(zij,zjj,ik)
                 ENDIF
              END DO
            END DO
         ENDIF
            ...
            SELECT CASE( igrd )                         
               CASE(1)
                  IF( ABS( (zh - ht(zij,zjj)) / ht(zij,zjj)) * tmask(zij,zjj,1) > 0.01_wp ) THEN
                     WRITE(ibstr,"(I10.10)") map%ptr(ib) 
                     CALL ctl_warn('fld_bdy_interp: T depths differ between grids at BDY point '//TRIM(ibstr)//' by more than 1%')
                 !   IF(lwp) WRITE(*,*) 'DEPTHT', zh, sum(e3t_n(zij,zjj,:), mask=tmask(zij,zjj,:)==1),  ht(zij,zjj), map%ptr(ib), ib, zij, zjj
                  ENDIF
               CASE(2)
                  IF( ABS( (zh - hu(zij,zjj)) * hur(zij,zjj)) * umask(zij,zjj,1) > 0.01_wp ) THEN
                     WRITE(ibstr,"(I10.10)") map%ptr(ib) 
