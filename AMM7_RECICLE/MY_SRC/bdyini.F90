...
904         IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambdy_index in configuration namelist', lwp )
      ...
      IF (nb_bdy>0) THEN
         ...
         IF( MAXVAL(nb_jpk_bdy)>0 ) THEN
            ALLOCATE( dta_global(jpbdtau, 1, MAXVAL(nb_jpk_bdy)) )
            ALLOCATE( dta_global_z(jpbdtau, 1, MAXVAL(nb_jpk_bdy)) )
            ALLOCATE( dta_global_dz(jpbdtau, 1, MAXVAL(nb_jpk_bdy)) )
         ELSE
         ...
         IF ( icount>0 ) THEN
            IF( MAXVAL(nb_jpk_bdy)>0 ) THEN
               ALLOCATE( dta_global2(jpbdtas, nrimmax, MAXVAL(nb_jpk_bdy)) )
               ALLOCATE( dta_global2_z(jpbdtas, nrimmax, MAXVAL(nb_jpk_bdy)) )
               ALLOCATE( dta_global2_dz(jpbdtas, nrimmax, MAXVAL(nb_jpk_bdy)) )
            ELSE
